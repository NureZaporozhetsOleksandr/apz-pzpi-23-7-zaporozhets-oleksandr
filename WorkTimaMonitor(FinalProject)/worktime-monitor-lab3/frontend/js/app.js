...
function syncDashboardFromLatestRecord() {
    ...
    const sortedRecords = [...state.records].sort((a, b) => {
        const timeA = new Date(a.startTime || a.StartTime || a.time || 0).getTime();
        const timeB = new Date(b.startTime || b.StartTime || b.time || 0).getTime();

        return timeB - timeA;
    });

    const latest = sortedRecords[0];
    const type = latest.entryType || latest.EntryType || latest.action;
