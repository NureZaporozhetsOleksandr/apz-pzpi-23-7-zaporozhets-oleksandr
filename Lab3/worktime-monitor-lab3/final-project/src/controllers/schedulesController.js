...
router.get('/', adminMiddleware, (req, res) => {
  const sql = `
    SELECT s.*, u.FullName, u.Login
    FROM WorkSchedules s
    LEFT JOIN Users u ON u.Id = s.UserId
    ORDER BY s.UserId IS NOT NULL, u.FullName
  `;
  db.all(sql, [], (err, rows) => {
...
router.post('/', adminMiddleware, (req, res) => {
  ...
  if (!startWork || !endWork || breakMinutes === undefined || !workingDaysMask) {
  ...
  db.run(sql, [userId || null, startWork, endWork, breakMinutes, workingDaysMask], function (err) {
    ...
    res.status(201).json({ id, userId: userId || null, startWork, endWork, breakMinutes, workingDaysMask });
  });
});
